...
namespace DigitalfileSyncService
{
    public partial class 图片识别工具 : Form
    {
        ...
        {
            InitializeComponent();
            txtContent.Text = "正在识别图片，请稍后...";
            txtHtmlContent.Text = "正在识别图片，请稍后...";
        }
        /// <summary>
        /// 确定事件处理
        /// </summary>
        ...
        private async void btnShibie_Click(object sender, EventArgs e)
        {
            ...
            #endregion
            //var resJson = Newtonsoft.Json.JsonConvert.DeserializeObject<Root>(t);
            ...
            try
            {
                using (HttpClient client = new HttpClient())
                {
                    ...
                    //获取当前程序所在的文件路径
                    // var pathname = @"C:\Users\Administrator\Desktop\IMG_0000145.jpg";
            ...
            {
                this.txtContent.Text = $"图片识别文本出错:{ex.Message}";
                //MessageBox.Show($"图片识别出错:{ex}");
            }
        }
        /// <summary>
        /// Html识别
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private async void btnHtmlReg_Click(object sender, EventArgs e)
        {
            #region 识别图片测试数据
           ...
           // var resJson = Newtonsoft.Json.JsonConvert.DeserializeObject<RootHtml>(t);
            ...
            try
            {
                using (HttpClient client = new HttpClient())
                {
                    ...
                    //获取当前程序所在的文件路径
                    //var pathname = @"C:\Users\Administrator\Desktop\TestFileScanProject\image0000000001A.jpg";
                    var request = new HttpRequestMessage(HttpMethod.Post, url);
                    content.Add(new StreamContent(System.IO.File.OpenRead(pathname)), "image", pathname);
                    request.Content = content;
                    request.Headers.Add("User-Agent", "ocr-html");
                    request.Headers.Add("Accept", "*/*");
                    var response = await client.SendAsync(request);
                    if (response.IsSuccessStatusCode)
                    {
                        var result = await response.Content.ReadAsStringAsync();
                        var res = JsonConvert.DeserializeObject<RootHtml>(result);
                        if (res.code != 200)
                        {
                            this.txtHtmlContent.Text = $"图片识别Html出错:{res.msg}";
                            return;
                        }
                        html = res.data;
            ...
            {
                this.txtHtmlContent.Text = $"图片识别Html出错:{ex.Message}";
